		...
		create_actors= function(self, params)
			...
			local frame= Def.ActorFrame{
				...
				InitCommand=function(subself)
					...
					self:diffuse(ColorLightTone(PlayerColor(self.pn)))
				end,
...
-- Seems like it should be a base for something but doesn't seem to be used.
...
-- Possibly some kind of fallback for if something doesn't set intended text correctly.
	...
	__index= {
		create_actors= function(self, params)
			...
			return Def.ActorFrame{
				...
				self.underline:create_actors{name= "underline", parts_name= "OptionsUnderline"},
				text_actor,
			}
		end,
...
-- A main menu item directly linked to some value or another.
local value_item_mt= {
	__index= {
		create_actors= function(self, params)
...
-- Just putting the two main menu item types into a little table?
nesty_items= {
		...
		interpret_code= function(self, code)
			...
			local funs= {
				MenuLeft= function(self)
					-- Practically a carbon copy of interpret_start...
					if self.interpret_mleft then
						local action, action_data= self:interpret_mleft()
	...
	__index= {
		initialize= function(self, pn, extra)
			...
			self.info_set= {
				...
				{
						text= "&MENULEFT; to decrease",
						sound= nil,
						type= "action"
				},
				{
						text= "&MENURIGHT; to increase",
						sound= nil,
						type= "action"
				},
				{
		...
		interpret_mright = function(self)
			self:set_new_val(self.current_value + self.min_scale)
			return true
		end,
...
nesty_menu_stack_mt= {
	__index= {
		...
		-- and "pop" the current menu out of existence.
		...
		return setmetatable(
			{
				...
				args= {
					...
					set= function(pn, obj, value)
						set_element_by_path(obj, field_name, value)
						MESSAGEMAN:Broadcast("ConfigValueChanged", {table_name= table_name, field_name= field_name, value= value})
					end,
				},
				value= function()
					return get_element_by_path(tab, field_name)
